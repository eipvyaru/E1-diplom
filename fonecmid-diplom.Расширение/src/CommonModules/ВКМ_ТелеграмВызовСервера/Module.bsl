...
Функция ОтправитьВгруппуТелеграм(ТекстСообщения) Экспорт
	...
	Попытка
		ЗаписьЖурналаРегистрации(НСтр("ru = 'ОтправитьВгруппуТелеграм'"), УровеньЖурналаРегистрации.Информация, , ,
		 НСтр("ru = 'Отправка в Телеграм.'"));
		ЗащищСоедин  = Новый ЗащищенноеСоединениеOpenSSL(Новый СертификатКлиентаWindows,
			Новый СертификатыУдостоверяющихЦентровWindows);

		Соедин = Новый HTTPСоединение("api.telegram.org", , , , , 300, ЗащищСоедин);
	...
	Исключение
		...
		ЗаписьЖурналаРегистрации(НСтр("ru = 'ОтправитьВгруппуТелеграм'"), УровеньЖурналаРегистрации.Ошибка, , , НСтр(
			"ru = 'Во время выполнения действия произошла ошибка.'") + Символы.ПС + ОшОп);
		Возврат Ложь;
	КонецПопытки;
	Возврат Истина;
КонецФункции
